...
import (
	...
	"github.com/scriptscat/cloudcat/internal/service/sync/domain/repository/mock"
)
...
func Test_sync_PushScript(t *testing.T) {
	...
	nils := &entity2.SyncScript{}
	id1 := &entity2.SyncScript{ID: 1}
	id2 := &entity2.SyncScript{ID: 2}
	tests := []struct {
		name    string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.EXPECT().FindById(gomock.Any()).Return(&entity2.SyncDevice{
				ID:     tt.args.device,
				UserID: tt.args.user,
			}, nil).AnyTimes()
